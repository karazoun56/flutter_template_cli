...
class PaginatedPage<T> with PaginatedPageMappable<T> {
  const PaginatedPage({
    required this.items,
    @MappableField(key: 'meta', hook: _TotalItemsHook()) required this.totalItems,
    @MappableField(key: 'meta', hook: _PageSizeHook()) required this.pageSize,
  });

  factory PaginatedPage.empty() => const PaginatedPage(
        items: [],
        totalItems: 0,
        pageSize: 0,
      );
  ...
  static PaginatedPage<T> fromJsonGeneric<T>(
    ...
    ensureInitialized();
    return PaginatedPageMapper.fromJson<T>(jsonEncode(json));
  }
}
...
class PaginatedItem<T> with PaginatedItemMappable<T> {
  ...
  static int calculateItemIndexInPage(int index, int pageSize) => index % pageSize;
  static int calculatePageIndex(int index, int pageSize) => index ~/ pageSize + 1;
}
